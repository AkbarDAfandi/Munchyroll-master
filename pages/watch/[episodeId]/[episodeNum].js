...
function StreamingPage({ episode, anime, episodeNumber }) {
  ...
  return (
    <>
      <Head>
        <title>{(anime?.title.english || anime?.title.romaji) + " Episode " + episodeNumber + " - Hu Tao Stream "}</title>
        <meta name="description" content={anime?.synopsis} />
        <meta name="keywords" content={anime?.genres} />
        <meta
          property="og:title"
          content={(anime?.title.english || anime?.title.romaji) + " Episode " + episodeNumber + " - Hu Tao Stream "}
        />
